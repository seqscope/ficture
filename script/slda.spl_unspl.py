...
logging.info(f"{M} features and {K} factors are read from input model")
...
input_header = ["random_index","X","Y","gene",spl,unspl]
unit_header = ["random_index","X","Y"]
clps_header = unit_header + ['gene', spl, unspl]
dty = {x:int for x in ['X','Y',spl,unspl]}
...
for it_epoch in range(args.epoch):
    ...
    while True:
        ...
        if not end_of_file:
            ...
            last_indx = chunk.random_index.iloc[-1]

            unspl_df = copy.copy(chunk.loc[chunk[unspl] > 0, :]).drop(columns = spl).rename(columns = {unspl:key})
            unspl_df['gene'] = 'unspl_' + unspl_df.gene.values
            unspl_df.loc[~unspl_df.gene.isin(ft_dict), 'gene'] = '_unspl'
            unspl_df = unspl_df.groupby(by=['random_index', 'gene'],as_index=False).agg({key:sum})
            unspl_df = unspl_df.merge(right=chunk.loc[:, unit_header].drop_duplicates(subset='random_index'), on='random_index', how='left')
            spl_df = copy.copy(chunk.loc[chunk[spl] > 0, :]).drop(columns = unspl).rename(columns = {spl:key})
            chunk = pd.concat([spl_df, unspl_df])
            chunk = chunk.loc[chunk.gene.isin(ft_dict), :]
            print(chunk.loc[chunk.gene.str.contains('unspl'), key].sum())

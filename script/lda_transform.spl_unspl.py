...
logging.info(f"Read existing model with {K} factors and {M} features. Attempting to use {lda.n_jobs} threads")
unspl_feature = set([x for x in feature_kept if 'unspl_' in x] )
unspl_idx = [i for i,x in enumerate(feature_kept) if 'unspl' in x]
...
spl = args.spl_key
unspl = args.unspl_key
key = 'tot'
if spl not in input_header or unspl not in input_header:
    sys.exit(f"Cannot find --spl_key or --unspl_key in the input file")
print(input_header)
...
logging.info(cmd)
n_unit = 0
dtp = {'topK':int,'j':str,'x':str,'y':str,spl:int,unspl:int}
dtp.update({x:float for x in ['topP']+factor_header})
dty = {x:int for x in ['X','Y',key,spl,unspl]}
...
for chunk in pd.read_csv(process.stdout,sep='\t',chunksize=chunk_size,\
                names=input_header, usecols=["#lane","X","Y","gene",spl,unspl], dtype=dty):
    last_chunk = chunk.shape[0] < chunk_size

    unspl_df = chunk.loc[chunk[unspl] > 0, :].drop(columns = [spl]).rename(columns = {unspl:key})
    unspl_df['gene'] = 'unspl_' + unspl_df.gene.values
    unspl_df.loc[~unspl_df.gene.isin(unspl_feature), 'gene'] = '_unspl'
    spl_df = chunk.loc[chunk[spl] > 0, :].drop(columns = [unspl]).rename(columns = {spl:key})
    spl_df = spl_df.loc[spl_df.gene.isin(ft_dict), :]
    chunk = pd.concat([unspl_df, spl_df])
    ...
    for l in l_list:
        ...
        while offs_x < n_move:
            while offs_y < n_move:
                ...
                row_sum = mtx.sum(axis = 1).squeeze()
                unspl_sum = mtx[:,unspl_idx].sum(axis = 1).squeeze()

                theta = lda.transform(mtx)
                post_count += np.array(theta.T @ mtx)

                # Output
                result = pd.DataFrame({"j":kept_unit, spl:row_sum-unspl_sum, unspl:unspl_sum })

...
ref_pts = outpath + "/kept_grid_pt.tsv"
    ...
    while st < nw:
        ...
        for i in range(n_move):
            for j in range(n_move):
                ...
                if m1 > m0 * .5:
                    cnt['det'] = 1
                indx = cnt.det.eq(lab_keep)
                if sum(indx) < 2:
                    continue
                anchor_x, anchor_y = hex_to_pixel(cnt.loc[indx, 'hex_x'].values, cnt.loc[indx, 'hex_y'].values, radius,i/n_move,j/n_move)

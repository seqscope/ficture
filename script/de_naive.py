...
grp = grp.loc[:, ['j','topK','topP',args.key]]
...
for chunk in pd.read_csv(gzip.open(args.input, 'rt'),sep='\t',chunksize=200000, header=0, usecols=[args.unit_col_name,"gene",args.key], dtype=adt):
    ...
    for k in range(K): # Count for bulk test
        ...
        v = pmtx.loc[chunk.j.values, str(k)].values * chunk[args.key].values
...
for name, v in info.iterrows():
    for k in range(K):
        ...
        tab=np.zeros((2,2))
        tab[0,0]=v[str(k)]
        tab[0,1]=v[args.key]-tab[0,0]
        tab[1,0]=total_k[k]-tab[0,0]
        tab[1,1]=tt-total_k[k]-v[args.key]+tab[0,0]
        ...
        res.append([name,k,chi2,p,fd,v[args.key],v['gene_id']])

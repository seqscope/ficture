...
use_existing_model = os.path.exists(args.use_model)
...
if use_existing_model:
    if args.use_model.endswith('.tsv.gz') or args.use_model.endswith('tsv'):
        model_mtx = pd.read_csv(args.use_model, sep='\t', index_col = 0)
        factor_header = list(model_mtx.columns)
        feature_kept =list(model_mtx.index)
        M, K = model_mtx.shape
        lda = LDA(n_components=K, learning_method='online', batch_size=512, n_jobs = args.thread, verbose = 0)
        init_latent_vars(lda, n_features = M, gamma = np.array(model_mtx).T)
    else:
        lda = pickle.load( open( args.use_model, "rb" ) )

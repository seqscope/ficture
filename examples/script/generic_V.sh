...
command time -v ficture slda_decode --input ${pixel} --output ${output} --model ${model} --anchor ${anchor} --anchor_in_um --neighbor_radius ${radius} --mu_scale ${mu_scale} --key ${key} --precision ${analysis_resolution} --lite_topk_output_pixel ${topk} --lite_topk_output_anchor ${topk} --thread ${thread}
...
ficture factor_report --path ${output_path} --pref ${prefix} --color_table ${cmap} --hc_tree
...
command time -v ficture plot_pixel_full --input ${input} --color_table ${cmap} --output ${output} --plot_um_per_pixel ${pixel_resolution} --full
...
if [ "${plot_subbatch}" -lt 1 ] || [ "${plot_subbatch}" -gt ${K} ]; then
    command time -v ficture plot_pixel_single --input ${input} --output ${output} --plot_um_per_pixel ${pixel_resolution} --full
else
    st=0
    ...
    while [ ${st} -lt ${K} ]; do
        ...
        command time -v ficture plot_pixel_single  --input ${input} --output ${output} --id_list ${id_list} --plot_um_per_pixel ${pixel_resolution} --full
